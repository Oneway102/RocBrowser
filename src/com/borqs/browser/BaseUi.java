...
import com.borqs.browser.Tab.SecurityState;
import com.borqs.browser.combo.ComboViewActivityMine;
...
public abstract class BaseUi implements UI {
	...
	protected void attachTabToContentView(Tab tab) {
		...
		ContentViewRenderView renderView = tab.getRenderView();
		...
		//2. attach the tab to content view.
	...
	@Override
	public void onSetWebView(Tab tab, ContentView webView) {
		View container = tab.getViewContainer();
		if (container == null) {
	 ...
	 */
	@Override
	public void createSubWindow(Tab tab, ContentView subView) {
		/*
        ...
        final ContentView cancelSubView = subView;
	 ...
	 */
	 ...
	 protected void updateLockIconToLatest(Tab t) {
		 if (t != null && t.inForeground()) {
			 //ww            updateLockIconImage(t.getSecurityState());
		 }
	 }
	 ...
	 public void setActiveTab(final Tab tab) {
		 ...
		 mActiveTab = tab;
		 ContentView web = (ContentView) mActiveTab.getWebView();
		 // ww        updateUrlBarAutoShowManagerTarget();
		 attachTabToContentView(tab);
		 if (web != null) {
			 // Request focus on the top window.
			 if (mUseQuickControls) {
				 //ww                mPieControl.forceToTop(mContentView);
				 // ww                web.setTitleBar(null);
				 mTitleBar.hide();
			 } else {
				 // ww                web.setTitleBar(mTitleBar);
				 mTitleBar.onScrollChanged();
			 }
		 }
		 mTitleBar.bringToFront();
		 tab.getTopWindow().requestFocus();
		 //ww        setShouldShowErrorConsole(tab, mUiController.shouldShowErrorConsole());
	 ...
	 public void showComboView(ComboViews startingView, Bundle extras) {
		 ...
		 if (SHOW_ORIGINAL) {
			 ...
			 Intent intent = new Intent(mActivity, ComboViewActivityMine.class);
			 intent.putExtra(ComboViewActivityMine.EXTRA_INITIAL_VIEW, startingView.name());
			 intent.putExtra(ComboViewActivityMine.EXTRA_COMBO_ARGS, extras);
			 Tab t = getActiveTab();
			 if (t != null) {
				 intent.putExtra(ComboViewActivityMine.EXTRA_CURRENT_URL, t.getUrl());
			 }
			 mActivity.startActivityForResult(intent, Controller.COMBO_VIEW);
