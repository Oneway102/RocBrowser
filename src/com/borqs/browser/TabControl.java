...
class TabControl {
     ...
     */
    ContentView getCurrentSubWindow() {
        Tab t = getTab(mCurrentTab);
        if (t == null) {
            return null;
        }
        return t.getSubContentView();
    }
    ...
    void addPreloadedTab(Tab tab) {
        ...
        mController.onSetContentView(tab, tab.getWebView());
        tab.putInBackground();
    }
...
*/
    ...
    void freeMemory() {
        ...
        ContentView view = getCurrentContentView();
        if (view != null) {
//ww            view.freeMemory();
        }
    }
     ...
     */
    Tab getTabFromView(ContentView view) {
        for (Tab t : mTabs) {
            if (t.getSubContentView() == view || t.getWebView() == view) {
                return t;
            }
        }
        return null;
    ...
    void stopAllLoading() {
        for (Tab t : mTabs) {
            final ContentView webview = t.getWebView();
            if (webview != null) {
                webview.stopLoading();
            }
            final ContentView subview = t.getSubContentView();
            if (subview != null) {
                webview.stopLoading();
            }
        }
    ...
    void recreateWebView(Tab t) {
        final ContentView w = t.getWebView();
        if (w != null) {
            t.destroy();
        }
        // Create a new WebView. If this tab is the current tab, we need to put
        // back all the clients so force it to be the current tab.
        t.setContentView(createNewWebView(), false);
        if (getCurrentTab() == t) {
     ...
     */
    private ContentView createNewWebView() {
     ...
     *        WebView.
     */
    private ContentView createNewWebView(boolean privateBrowsing) {
        return mController.getContentViewFactory().createContentView(privateBrowsing);
    }
